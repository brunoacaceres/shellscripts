...
PUBLICAHOST="`/bin/find "$UPLOADPUBLICA" -name "*bin" |  /bin/awk -F "/" '{print $4}' | /bin/egrep -o ^[A-Z]*| /usr/bin/uniq`"

PIDPUB=`ps ax | grep "/opt/publica/deployhostsitnew.sh" | grep -v grep | awk -F " " '{print $1}'`
...
then
        ...
        do
                ...
                then
                        ...
                        APPSPUBLICA="`/bin/ls "$UPLOADPUBLICA/$AMBIENTE/sgv/host/" | grep bin |   /bin/awk -F "-" '{print $0}' `"
                        HOMEPUBLICA=/opt/$APPAMBIENTE
...
#                         fi
				...
				HOSTF="172.17.61.1"
                        ...
                        do
                                APPNAME="`echo ${j} | /bin/awk -F "-" '{if ($3 == "tef" || $3 == "v" || $3 == "first" ) {print$1"-"$2"-"$3;}else{print$1"-"$2};}'`"
                                APP="`echo ${j} | /bin/awk -F "-" '{if ($3 == "tef" || $3 == "v" || $3 == "first" ) {print$2"-"$3;}else{print$2};}'`"
...
#                                                "`$CMDUSER "/usr/bin/ssh $HOST6 -C rm -f $HOMEPUBLICA/$j"`"
                                ...
                                do
                                        ...
                                        then
                                               ...
                                               echo "`date +%c` -  Iniciando publicacao do server $HOSTF - $APPNAME - $AMBIENTE"
                                               echo "`date +%c` - Removendo deploy anterior da $APPNAME remota"
                                               "`$CMDUSER "/usr/bin/ssh $HOSTF -C rm -f $HOMEPUBLICA/$APPNAME*bin"`"

                                               echo "`date +%c` -  Copiando binario ${j} para a pasta de publicacao"
                                               `$CMDUSER "/usr/bin/scp $UPLOADPUBLICA/$AMBIENTE/sgv/host/$j $HOSTF:$HOMEPUBLICA"`

                                               #Verificando PID da APP a ser publicada
                                               PID=`$CMDUSER "/usr/bin/ssh $HOSTF -C ps ax | grep $APP$INST-$APPAMBIENTE | grep -v "grep" | sed s/^[\ ]*//g | egrep -o ^[0-9]*"`

                                                if [ "$PID" != "" ]
                                                then
                                                        echo "`date +%c` - Parando aplicacao $APPNAME"
                                                        `$CMDUSER "/usr/bin/ssh $HOSTF -C kill -9 $PID 2> /dev/null"`
                                                        `$CMDUSER "/usr/bin/ssh $HOSTF -C rm -f /var/run/$APP$INST-$APPAMBIENTE.pid"`
                                                else
                                                        echo "`date +%c` -  A App $APPNAME ja esta parada"
                                                fi

                                                echo "`date +%c` -  Publicando aplicacao"
                                                `$CMDUSER "/usr/bin/ssh $HOSTF -C $HOMEPUBLICA/$j $HOMEPUBLICA $HOMEPUBLICA/conf/$APPNAME-ATUAL.properties"`
                                                echo
                                                echo "`date +%c` -  Publicacao  $APPNAME instalada no ambiente $AMBIENTE"
                                                ...
                                                echo "`date +%c` -  Iniciando $APPNAME"
                                                `$CMDUSER "/usr/bin/ssh $HOSTF -C $STARTSGV $APP $APPAMBIENTE start"`

                                                 #Verificando se aplicacao esta ativa apos deploy
                                                 sleep 5
                                                 NOVOPID=`$CMDUSER "/usr/bin/ssh $HOSTF -C ps ax | grep $APP$INST-$APPAMBIENTE | grep -v "grep" | sed s/^[\ ]*//g | egrep -o ^[0-9]*"`
                                                  PIDRUN=`$CMDUSER "/usr/bin/ssh $HOSTF -C cat /var/run/$APP$INST-$APPAMBIENTE.pid"`
