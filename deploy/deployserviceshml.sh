...
ENVIAEMAIL="infra.java@redetendencia.com.br emanuel.melechco@redetendencia.com.br erico.costa@redetendencia.com.br thiago.michelini@redetendencia.com.br fabrica.qualidade@redetendencia.com.br "
...
PIDPUB=`ps ax | grep "/opt/publica/deployserviceshml.sh" | grep -v grep | awk -F " " '{print $1}'`	

#PIDHML="`cat $PUBLICAHOME/PIDHML.pid`"
PIDSERVICESHML="`cat $PUBLICAHOME/PIDSERVICESHML.pid`"

echo "PUBLICA WEB" $PUBLICAWEB
echo "o PID DO PUBLICADOR" $PIDPUB
echo "o PID SERVICES HML EH " $PIDSERVICESHML
...
then
        ...
        do
              ...
              SGVWEBNOVO="`/bin/ls "$PUBLICAHOME/$AMBIENTE/services/" | egrep "*\.[ew]ar" | /bin/awk -F "-" '{print $0}'| /bin/sort | /usr/bin/tail -n1`"
              VERSAONOVA=`echo "$SGVWEBNOVO" |  /bin/awk -F "-" '{if($1=="soa"){print$3;}else{print$2};}' | sed 's/.[we]ar//g'`

               if [ "$AMBIENTE" = "SIT" ] || [ "$AMBIENTE" = "UAT" ] || [ "$AMBIENTE" = "INT" ] && [ "$PIDSERVICESHML" = "" ] && [ "$SGVWEBNOVO" != "" ] 
                then
                        EXPURGO_LOGS=/opt/publica/log/"Publicacao-SERVICES-HML-`date +%Y%m%d`.log"
                        LOGS=/opt/publica/log/"Relatorio-SERVICES-HML.log"
#                        #Armazenando logs
                        exec > $LOGS 2>&1
                        echo "----------------------INICIO------------------------"
			
                        echo $PIDPUB > $PUBLICAHOME/PIDSERVICESHML.pid
                        PIDSERVICESHML="`cat $PUBLICAHOME/PIDSERVICESHML.pid`"
                        echo "o PID EH " $PIDSERVICESHML
			...
			echo "Listando pasta lib corrente=" $LIB
				...
				echo "Restart nao necessario pois as libs estao corretas"
                        ...
                        #SGVWEBNOVO="`/bin/ls "$PUBLICAHOME/$AMBIENTE/services/" | grep "*\.[ew]ar" | /bin/awk -F "-" '{print $0}'| /bin/sort | /usr/bin/tail -n1`"
			#VERSAONOVA=`echo "$SGVWEBNOVO" |  /bin/awk -F "-" '{if($1=="tnd"){print$4;}else{print$3};}' | sed 's/.[we]ar//g'`
				...
				APP=`echo "$APPSWEB" | tr "a-z" "A-Z"`
                                ...
                                TIPO=`echo $APP | /bin/awk -F "-" '{if($1=="SOA"){print$2;}else{print$2};}'`
                                ...
                                `$CMDUSER "/usr/bin/scp  "$PUBLICAHOME/$AMBIENTE/services/$APPSWEB*" $IP1:$PUBLICAHOME/$AMBIENTE/services/"`
                                echo
          ...
          INSTATIVA=`$CMDUSER "/usr/bin/ssh $IP1 -C $GFHOME/bin/asadmin  --passwordfile $GFCONF/pwd.conf $PORT list-instances | grep $INSTANCIA" | awk -F " " '{print $2}' | grep -c running`
				...
				CONTEXTSERVICES=`echo $APPSWEB | /bin/awk -F "-" '{print $1}'`  #"-"$2}'`
                                if [ $CONTEXTSERVICES = "F3M" ]
                                then
					#NAMECONTEXT="--name  $APPSWEB-$VERSAONOVA-$AMBIENTE --contextroot tnd-sp-vivo"
					NAMECONTEXT="--name  $APPSWEB-$AMBIENTE  --contextroot F3M"
                               	else
					#NAMECONTEXT="--name $APPSWEB-$VERSAONOVA-$AMBIENTE"
					NAMECONTEXT="--name $APPSWEB-$AMBIENTE"
                                fi

				  "`$CMDUSER "/usr/bin/ssh $IP1 -C $GFHOME/bin/asadmin  --passwordfile $GFCONF/pwd.conf $PORT deploy $NAMECONTEXT  --target $INSTANCIA  $PUBLICAHOME/$AMBIENTE/services/$APPWEBNOVA   "`"
...
#                                DPLPUB=`$CMDUSER "/usr/bin/ssh $IP1 -C /bin/find $GFHOME/domains/$DOMAIN/applications/__internal/$APPSWEB-$VERSAONOVA-$AMBIENTE/ -name '$APPSWEB*\.[ew]ar'"`
				 ...
				 if [ "$DPLPUB" != "" ]
					...
					`$CMDUSER "/usr/bin/ssh $IP1 -C  $GFHOME/bin/asadmin --passwordfile $GFCONF/pwd.conf $PORT start-local-instance --sync full $INSTANCIA "`
                                        ...
                                        MD5DPLNOV=`$CMDUSER "/usr/bin/ssh $IP1 -C /usr/bin/md5sum $PUBLICAHOME/$AMBIENTE/services/$APPSWEB*" | /bin/awk -F " " '{print $1}'`
...
#                                        echo
                                        ...
                                        "`$CMDUSER "/usr/bin/ssh $IP1 -C rm -f $PUBLICAHOME/$AMBIENTE/services/$APPSWEB*"`"
#                                        echo
                                        echo "`date +%c` - Removendo deploy $SGVWEBNOVO local"
                                        "`rm -f $PUBLICAHOME/$AMBIENTE/services/$APPSWEB*`"
#                                        echo

                                        else
						echo "Deploy nao realizado"
                                                `$CMDUSER "/usr/bin/php /var/www/html/retornopublica.php $SISTEMA-$AMBIENTE-WEB $APPWEBNOVA ERROR"`
                                                echo "`date +%c` - Removendo  deploy $SGVWEBNOVO remoto"
                                                "`$CMDUSER "/usr/bin/ssh $IP1 -C rm -f $PUBLICAHOME/$AMBIENTE/services/$APPSWEB*"`"
                                                echo
                                                echo "`date +%c` - Removendo deploy $SGVWEBNOVO local"
                                                "`rm -f $PUBLICAHOME/$AMBIENTE/services/$APPSWEB*`"
                                                `echo "" ` > "$PUBLICAHOME/PIDSERVICESHML.pid"
